            ...
            while (true) {
                synchronized (CircularByteBuffer.this) {
                    ...
                    if (available > 0) {
                        ...
                        return result;
                    } else if (outputStreamClosed) return -1;
                }
            ...
            while (true) {
                synchronized (CircularByteBuffer.this) {
                    ...
                    if (available > 0) {
                        ...
                        if (secondLen > 0) readPosition	= secondLen;
                        else readPosition	+= length;
                        if (readPosition == buffer.length) readPosition	= 0;
                        ensureMark();

                        return length;
                    } else if (outputStreamClosed) return 0;
                }
        ...
        public void write(final byte[] cbuf, int off, int len) throws IOException {
            while (len > 0) {
                synchronized (CircularByteBuffer.this) {
                    ...
                    if (firstLen > 0) System.arraycopy(cbuf, off, buffer, writePosition, firstLen);
