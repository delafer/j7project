...
//
//      return !className.endsWith(ModulesLoader.BL_DOT) ? rightFile(className) : false;
//   }
   ...
   private final void find0(String packageName) throws IOException {
      ...
      while (urls.hasMoreElements()) {
         ...
         String urlString = URLDecoder.decode(packageUrl.getFile(), ModulesLoader.CHARSET);
   ...
   public void checkClass(String className) {
	 ...
	 try {
		 //Class<?> ldClass =   Thread.currentThread().getContextClassLoader().loadClass(className);
		 ClassLoader loader =  Thread.currentThread().getContextClassLoader();
//		 ClassLoader loader = ClassLoader.getSystemClassLoader();
//		 ClassLoader loader = this.getClass().getClassLoader();
		 Class<?> ldClass = Class.forName(className, false,  loader);
		 checkClass(ldClass);
	} catch (java.lang.NoClassDefFoundError u) {
...
//		logger.warn("Can't load class: "+className);
   ...
   public abstract void checkClass(Class<?> ldClass);
...
/**
      ...
      for (File childFile : packageDirectory.listFiles()) {
         ...
         if (childFile.isDirectory()) {
            ...
            findFileNamesRecursive(childFile, subBuilder, subBuilder.length());
             ...
             {
            	 qnb.setLength(qualifiedNamePrefixLength);
            	 qnb.append(simpleClassName);
            	 checkClass(qnb.toString());
             }
         }
      }
   }
   ...
   private void processArchive(String pathWithPrefix, ZipFile jarFile) throws IOException {
      ...
      while (jarEntryEnumeration.hasMoreElements()) {
         ...
         if (absoluteFileName.startsWith(ModulesLoader.SLASH)) {
         ...
         if (absoluteFileName.startsWith(ModulesLoader.WARCLS_PATH)) {
