...
//		Security.addProvider(new IAIK());
	...
	public static void main(String[] args) {
		...
		int x = 0;
		byte[] test = randomKey(1024 * 1024 * 10);
		int keySize = -1;
		Akey key = null;
		TextWriter w = null;
		try {
	...
	private static Akey doTest(byte[] test, TextWriter w, int pass, Provider provider, String alg)
		...
		int keySize;
		Akey key;
		Cipher c = alg.contains("PKCS5Padding") ? Cipher.getInstance(alg) : Cipher.getInstance(alg, provider.getName());
		Cipher d = alg.contains("PKCS5Padding") ? Cipher.getInstance(alg) : Cipher.getInstance(alg, provider.getName());
		...
		if (key.pubKey != null) {
			initCipher(key, c, d, alg);
		}
		else
			if (key.cert != null) {
				c.init(Cipher.ENCRYPT_MODE, key.cert);
				d.init(Cipher.DECRYPT_MODE, key.cert);
			}
		...
		double speed = (1000d / (double)time) * 50D;
