...
final class StreamingXXHash64JavaSafe extends AbstractStreamingXXHash64Java
{
    ...
    public long getValue() {
        ...
        if (off <= this.memSize - 4) {
            h64 ^= (SafeUtils.readIntLE(this.memory, off) & 0xFFFFFFFFL) * PRIME64_1;
    ...
    public void update(final byte[] buf, int off, final int len) {
        ...
        if (this.memSize > 0) {
            ...
            this.v1 += SafeUtils.readLongLE(this.memory, 0) * PRIME64_2;
            this.v1 = Long.rotateLeft(this.v1, 31);
            this.v1 *= PRIME64_1;
            this.v2 += SafeUtils.readLongLE(this.memory, 8) * PRIME64_2;
            this.v2 = Long.rotateLeft(this.v2, 31);
            this.v2 *= PRIME64_1;
            this.v3 += SafeUtils.readLongLE(this.memory, 16) * PRIME64_2;
            this.v3 = Long.rotateLeft(this.v3, 31);
            this.v3 *= PRIME64_1;
            this.v4 += SafeUtils.readLongLE(this.memory, 24) * PRIME64_2;
        ...
        long v3;
        long v4;
        for (v2 = this.v2, v3 = this.v3, v4 = this.v4; off <= limit; off += 8, v2 += SafeUtils.readLongLE(buf, off) * PRIME64_2, v2 = Long.rotateLeft(v2, 31), v2 *= PRIME64_1, off += 8, v3 += SafeUtils.readLongLE(buf, off) * PRIME64_2, v3 = Long.rotateLeft(v3, 31), v3 *= PRIME64_1, off += 8, v4 += SafeUtils.readLongLE(buf, off) * PRIME64_2, v4 = Long.rotateLeft(v4, 31), v4 *= PRIME64_1, off += 8) {
