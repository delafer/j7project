...
public class TestDownscale {
	...
	public static void main(String[] args) {
		try {
			...
			BufferedImage image = ImageIO.read(new File("A:\\target\\src\\downscale.jpg"));
...
//				for (int j = 0; j < 7; j++) {
					BufferedImage bi = r.resize(image, 2875, 2160);
...
//				}
				...
				ImageIO.write(bi, "png", new File(String.format("A:\\downscale1\\%s.png", r)));
			...
			for (ResizerBase filter : filters) {
				...
				long t1 = System.currentTimeMillis();
//				for (int j = 0; j < 7; j++) {
					BufferedImage bi = filter.resize(image, 2875, 2160);
					hc += System.identityHashCode(bi);
//				}
				long t2 = System.currentTimeMillis();
				System.out.println(String.format("Method: %s  time: %s", filter, (t2-t1)));
				byKey((t2-t1)).add(String.format("%s", filter));
				ImageIO.write(bi, "png", new File(String.format("A:\\downscale1\\%s.png", filter)));
			}
			for (int i = 0; i < 4; i++) {
				System.out.println();
			}
