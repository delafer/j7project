...
public class TestSpeed {
	...
	public static void main(String[] args) {
		try {
			...
			BufferedImage image = ImageIO.read(new File("A:\\resize.jpg"));
			...
			for (int i = 1; i <= 11 ; i++) {
				...
				for (int j = 0; j < 7; j++) {
					BufferedImage bi = r.resize(image, 3240, 2160);
					if (bi.getWidth()!=3240 || bi.getHeight()!=2160) {
			...
			for (ResizerBase filter : filters) {
				...
				long t1 = System.currentTimeMillis();
				for (int j = 0; j < 7; j++) {
					BufferedImage bi = filter.resize(image, 3240, 2160);
					hc += System.identityHashCode(bi);
					if (bi.getWidth()!=3240 || bi.getHeight()!=2160) {
						System.out.printf("!!ERROR!!");
					}
