...
public class ResampleOp extends AdvancedResizeOp
{
	...
	private final byte bMAX_CHANNEL_VALUE= (byte)MAX_CHANNEL_VALUE;
...
//        m.measure("threadsWork");
    ...
    private void verticalFromWorkToDst(byte[][] workPixels, byte[] outPixels, int start, int delta) {
        ...
        {
			final int xLocationStr = x*nrChannels;
			for (int y = dstHeight-1; y >=0 ; y--)
			{
				...
				for (int j= (--max); j >=0 ; j--) {
					...
					sample1+= (workPixels[valueLocation][++xLocation]&0xff) * arrWeight;
					sample2+= (workPixels[valueLocation][++xLocation]&0xff) * arrWeight;
					if (useChannel3){
						sample3+= (workPixels[valueLocation][++xLocation]&0xff) * arrWeight;
					}
				...
				outPixels[++sampleLocation ] = toByte(sample1);
				outPixels[++sampleLocation ] = toByte(sample2);
				if (useChannel3){
					outPixels[++sampleLocation ] = toByte(sample3);
				}
		...
		for (int k = start; k < srcHeight; k+=delta)
        {
			...
			for (int i = dstWidth-1;i>=0 ; i--)
			{
				...
				for (int j= (--max); j >= 0; j--) {
					...
					sample1 += (srcPixels[++pixelIndex]&0xff) * arrWeight;
					sample2 += (srcPixels[++pixelIndex]&0xff)  * arrWeight;
					if (useChannel3){
						sample3 += (srcPixels[++pixelIndex]&0xff)  * arrWeight;
					}
					index++;
				}

				workPixels[k][sampleLocation] = toByte(sample0);
				workPixels[k][++sampleLocation] = toByte(sample1);
				workPixels[k][++sampleLocation] = toByte(sample2);
				if (useChannel3){
					workPixels[k][++sampleLocation] = toByte(sample3);
				}
			}
			processedItems++;
			if (start==0){ // only update progress listener from main thread
    ...
    private float flMAX_CHANNEL_VALUE = (float)MAX_CHANNEL_VALUE;
...
//	}
	private final byte toByte(final float f){

		return f < 0f ? 0 : f <= flMAX_CHANNEL_VALUE ? (byte)(f+.5f) : bMAX_CHANNEL_VALUE;
